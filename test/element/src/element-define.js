...
const html = `
    <html>
        <head>
            <script type="module" src="/lottie_svg.min.js"></script>
        </head>
        <body>
            <lord-icon src="/icons/lock.json"></lord-icon>
            <script type="module">
                import { defineElement } from '/lordicon.js';
...
export default function () {
    ...
    describe("define", () => {
        ...
        it("not defined", () => {
            expect(
                iframe.contentWindow.document.querySelector("lord-icon").isReady
            ).to.be.undefined;
            expect(
                iframe.contentWindow.document.querySelector("lord-icon")
                    .offsetWidth
            ).to.be.eql(0);
        });
